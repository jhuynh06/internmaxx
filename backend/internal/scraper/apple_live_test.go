...
func TestAppleLive(t *testing.T) {
	...
	c := registry.Company{Name: "Apple", ATS: "apple", Slug: "apple", Tier: 1, Group: "software"}
	jobs, err := Apple{}.Fetch(ctx, NewClient(300*time.Millisecond), c)
	if err != nil {
		t.Fatalf("Fetch: %v", err)
	}
	if len(jobs) == 0 {
		t.Fatal("expected at least one Apple posting")
	}
